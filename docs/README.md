...
- [feed-backend.md](./feed-backend.md) — the fan-out-on-read home feed (`GET /api/v1/feed`): own-tweets-included author-set resolution, keyset pagination/tie-breaking, the isolated short-TTL first-page Redis cache, N+1-free batch view assembly, and query-plan/concurrency test evidence (`TSC-FEED-001`).
- [frontend-feed.md](./frontend-feed.md) — the responsive infinite-scrolling home feed: `useFeed`/`useRefreshFeed`, the reusable `IntersectionObserver`-based pagination hook, refresh semantics, the hand-rolled scroll-restoration policy, and Vitest/Playwright test evidence (`TSC-FEED-002`).
- [likes-backend.md](./likes-backend.md) — idempotent like/unlike, the atomic non-negative `like_count` counter update, the like → notification side effect with self-like never notifying, like rate limiting, the periodic like/reply counter-reconciliation Celery task, and concurrency/idempotency test evidence (`TSC-LIKE-001`).
